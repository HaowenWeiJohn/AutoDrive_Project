...
def depth_onehot(matrix, category, on_value=1, off_value=0, channel_first=True):
    x = matrix.shape[0]
    y = matrix.shape[1]
    if not channel_first:
        ...
        output = np.full((len(category), x, y), off_value)
        for ix, iy in np.ndindex(matrix.shape):
            value = matrix[ix, iy]
            output[category.index(value), ix, iy] = on_value
        return output
