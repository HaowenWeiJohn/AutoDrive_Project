...
class Spatial_path(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.context1 = ResContextBlock(in_filters=13, out_filters=32)
        self.context2 = ResContextBlock(in_filters=32, out_filters=32)
        self.convblock1 = ConvBlock(in_channels=32, out_channels=64)
        self.convblock2 = ConvBlock(in_channels=64, out_channels=128)
        self.convblock3 = ConvBlock(in_channels=128, out_channels=256)

    def forward(self, input):
        x = self.context1(input)
        x = self.context2(x)
        x = self.convblock1(x)
        x = self.convblock2(x)
        x = self.convblock3(x)
