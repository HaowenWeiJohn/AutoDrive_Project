...
optimizer = torch.optim.AdamW(ResLSTM_model.parameters(), lr=1e-5,weight_decay=1e-9)
...
#################
# training logger
training_logger = Training_Logger(root_dir=root_save_dir, logger_dir='ResLSTM_new_machine')
...
for current_epoch in range(0, 60):
    ...
    for (semantic_input, semantic_label) in looper:
        ...
        output = ResLSTM_model(semantic_input)
    ...
    if current_epoch % validation_step == 0:  # make one val after the first epoch
        iou_mean, iou, acc = val(model=ResLSTM_model, dataloader=val_loader)
        ...
        if iou_mean > max_miou:
            max_miou = iou_mean
            training_logger.save_model(model=ResLSTM_model)
